...
`js/lesson-colors-page.js`, `js/lesson-calendar-page.js`, and
`js/lesson-time-page.js` are each lesson's page-specific glue: fetch
...
face, and a study-time-of-day picker).
...
Days/Months/Seasons, and Time then all shipped using `initPicker()`
exactly as it already stood, with no further extension needed:
...
seven lessons pass it straight to `recordQuizResult()`
